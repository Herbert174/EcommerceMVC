<?php
    ...
    class TransacaoDAO
        ...
        public function ResgatarFiltroTransacoes(TransacaoVO $Transacao)
            {
            $objDb = new database();//Instância objDb com as propriedades database de db.class
            $link = $objDb -> conecta_mysql();//Cria uma conexão com o banco de dados e armazena em link
        
            $NomeComprador = $Transacao->retornaNomeComprador();
            $StatusPagamento = $Transacao->retornaStatusPagamento();

            $lista = [];//Cria um array para armazenar o resultado da leitura do banco de dados

            if($NomeComprador == '' && $StatusPagamento != '')
                {
                $sql = " SELECT codigo_referencia, nome_comprador, status_pagamento, codigo_transacao, valor_compra FROM transacoes WHERE status_pagamento = '$StatusPagamento' ORDER BY id_transacao DESC ";
                }

            if($NomeComprador != '' && $StatusPagamento == '')
                {
                $sql = " SELECT codigo_referencia, nome_comprador, status_pagamento, codigo_transacao, valor_compra FROM transacoes WHERE nome_comprador = '$NomeComprador' ORDER BY id_transacao DESC ";
                }

            if($NomeComprador != '' && $StatusPagamento != '')
                {
                $sql = " SELECT codigo_referencia, nome_comprador, status_pagamento, codigo_transacao, valor_compra FROM transacoes WHERE nome_comprador = '$NomeComprador' AND status_pagamento = '$StatusPagamento' ORDER BY id_transacao DESC ";
                }  
        ...
        public function RecuperarCompradores()
            {
            $objDb = new database();//Instância objDb com as propriedades database de db.class
            $link = $objDb -> conecta_mysql();//Cria uma conexão com o banco de dados e armazena em link
        
            $lista = [];//Cria um array para armazenar o resultado da leitura do banco de dados
            $sql = " SELECT nome_comprador FROM transacoes ORDER BY id_transacao DESC ";//Armazena em sql o codigo que será enviado para o banco de dados
